  ...
  return (
    ...
    >
      <motion.article
        ...
        className={`group h-full overflow-hidden rounded-[28px] border transition-colors duration-300 hover:border-gold/45 ${
        light ? "border-[rgba(77,80,57,0.18)] bg-[#F4F1E8] text-[#202315]" : "border-forest/40 bg-forest/20 text-cream"
        }`}
      >
      <div className={`relative overflow-hidden bg-forest/30 ${large ? "aspect-[16/10] md:aspect-[16/9]" : "aspect-[16/10]"}`}>
        <img
          src={imageUrl}
          alt={imageAlt}
          loading="lazy"
      ...
      <div className={`p-6 ${large ? "md:p-8" : ""}`}>
        ...
        <h3 className={`mb-3 font-sans leading-tight transition-colors group-hover:text-olive ${light ? "text-deep" : "text-cream"} ${large ? "text-3xl md:text-4xl" : "text-2xl"}`}>
          {title}
        </h3>
        <p className={`line-clamp-2 font-sans text-sm leading-relaxed ${light ? "text-[#5F6648]" : "text-sage/80"}`}>{excerpt}</p>
      </div>
      </motion.article>
    </Link>
  );
