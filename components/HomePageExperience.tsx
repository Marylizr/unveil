...
export default function HomePageExperience({
  ...
  return (
    <>
      ...
      <div className="overflow-hidden border-y border-gold/20 bg-pale-gold py-4">
        <div className="marquee-track">
          {marqueeItems.map((phrase, i) => (
            <span key={i} className="whitespace-nowrap px-10 font-serif text-sm italic text-deep/70">
              {phrase}
        ...
        <div className="mx-auto max-w-7xl">
          <div className="reveal mb-14 grid grid-cols-1 gap-8 md:grid-cols-[0.78fr_1.22fr] md:items-end">
            <div>
              ...
              <h2 className="font-serif text-5xl leading-tight text-deep md:text-6xl">Authority through education</h2>
            </div>
              ...
              {placeholderPosts.map((p, i) => (
                ...
                >
                  ...
                  <div className={i === 0 ? "p-[clamp(1.5rem,3vw,2.5rem)]" : "p-6"}>
                    <p className="mb-3 font-sans text-[10px] uppercase tracking-widest text-gold">{p.category}</p>
                    <h3 className={`font-sans font-semibold leading-tight text-deep ${i === 0 ? "text-3xl md:text-4xl" : "text-2xl"}`}>
                      {p.title[language]}
                    </h3>
                    <p className="mt-4 line-clamp-3 font-sans text-sm leading-relaxed text-[#5F6648]">{p.excerpt[language]}</p>
                  </div>
                </article>
              ))}
            </div>
